...
function renderTestsTab(container) {
  container.innerHTML =
    '<div class="section-title">Активные тесты</div>' +
...
function showSkillLevels(skillId) {
  ...
  var html = levels.map(function(lvl) {
    ...
    if (isCur) {
      ...
      nameStyle = 'text-decoration:line-through;color:#c0c0c0'; descStyle = 'color:#d0d0d0'; hoursStyle = 'color:#d0d0d0';
    } else if (isNext) {
      nameStyle = 'font-weight:600;color:' + skill.color; descStyle = 'color:' + skill.color + ';opacity:0.75'; hoursStyle = 'color:' + skill.color;
    } else {
      nameStyle = 'color:#c8c8c8'; descStyle = 'color:#d8d8d8'; hoursStyle = 'color:#d8d8d8';
    }
